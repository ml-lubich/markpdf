...
import { ok, err, isOk, isErr, unwrap, unwrapOr, unwrapOrElse, Ok, Err } from '../../lib/domain/Result';
...
test('ok() should handle objects', (t) => {
	...
	t.true(result.isOk());
	t.deepEqual(result.value, obj);
	t.deepEqual(unwrap(result), obj);
});
...
test('ok() should handle arrays', (t) => {
	...
	t.true(result.isOk());
	t.deepEqual(result.value, arr);
	t.deepEqual(unwrap(result), arr);
});
...
test('should handle large arrays', (t) => {
	...
	t.true(result.isOk());
	t.is(unwrap(result).length, 10000);
});
...
test('should handle large strings', (t) => {
	...
	t.true(result.isOk());
	t.is(unwrap(result).length, 100000);
});
